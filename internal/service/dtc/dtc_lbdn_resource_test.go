...
func testAccCheckDtcLbdnExistsUDDI(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		apiRes, _, err := acctest.UDDIClient.DNSTrafficControlAPI.PolicyAPI.Read(context.Background(), rs.Primary.ID).Execute()
		if err != nil {
			return fmt.Errorf("failed to read DtcLbdn: %w", err)
		}
		if !apiRes.HasResult() {
...
func testAccCheckDtcLbdnDestroyUDDI(resourceType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			_, httpRes, err := acctest.UDDIClient.DNSTrafficControlAPI.PolicyAPI.Read(context.Background(), rs.Primary.ID).Execute()
			if err != nil {
				if httpRes != nil && httpRes.StatusCode == http.StatusNotFound {
					return nil
				}
...
func testAccCheckDtcLbdnDisappearsUDDI(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		_, err := acctest.UDDIClient.DNSTrafficControlAPI.PolicyAPI.Delete(context.Background(), rs.Primary.ID).Execute()
		if err != nil {
			return fmt.Errorf("failed to delete DtcLbdn: %w", err)
		}
		return nil
