...
func (s *dtcLbdnService) createUDDI(ctx context.Context, obj *dtc.DtcLbdn, opts *core.Options) (*dtc.DtcLbdn, *http.Response, error) {
	payload, err := common.MapTo[uddidtc.Policy](obj, mapper.DtcLbdnUDDIFieldMap)
	if err != nil {
		return nil, nil, err
	}

	req := s.uddiClient.DNSTrafficControlAPI.PolicyAPI.
		Create(ctx).
		Body(payload)
...
func (s *dtcLbdnService) deleteUDDI(ctx context.Context, id string) (*http.Response, error) {
	httpResp, err := s.uddiClient.DNSTrafficControlAPI.PolicyAPI.
		Delete(ctx, id).
		Execute()
	return httpResp, err
}
...
func (s *dtcLbdnService) listUDDI(ctx context.Context, opts *core.ListOptions) ([]*dtc.DtcLbdn, *http.Response, string, error) {
	req := s.uddiClient.DNSTrafficControlAPI.PolicyAPI.List(ctx)
...
func mapUDDIDtcLbdnToResponse(r *uddidtc.Policy) *dtc.DtcLbdn {
	resp := &dtc.DtcLbdn{
		Id: r.Id,
	}
